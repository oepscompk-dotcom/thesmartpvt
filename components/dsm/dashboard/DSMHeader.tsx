...
export default function DSMHeader({ onMenuClick }: Props) {
  const { settings, auth, notifications, dsmLogout } = useDSMData();
  ...
  const userName = settings.dsmName || auth.dsmName || "DSM";
    ...
    <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30">
      <div className="flex items-center gap-4">
        ...
        <div className="hidden sm:flex items-center gap-2 bg-slate-50 rounded-xl px-4 py-2.5 w-80 border border-slate-200 focus-within:border-brand-500/30 focus-within:ring-2 focus-within:ring-brand-500/10 transition-all">
          ...
          <input type="text" placeholder="Search DSOs, activations..." className="bg-transparent text-slate-900 text-sm placeholder:text-slate-400 focus:outline-none w-full" />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <UserMenu
          ...
          id={userId}
          links={[
            { label: "Settings", href: "/dsm/profile", icon: Settings },
            { label: "Profile", href: "/dsm/profile", icon: User },
            { label: "Notifications", href: "/dsm/notifications", icon: Bell, unread },
            { label: "Sign out", icon: LogOut, danger: true, onClick: () => dsmLogout() },
          ]}
        />
      </div>
    </header>
