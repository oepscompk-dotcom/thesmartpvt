        ...
        {step === "select" ? (
          <>
            ...
            <input
              ...
              className="w-full px-4 py-4 bg-white/5 border border-white/10 rounded-xl text-white text-center text-lg font-mono tracking-widest placeholder:text-white/25 placeholder:tracking-normal focus:outline-none focus:border-[#0EA5E9]/50 focus:ring-2 focus:ring-[#0EA5E9]/10 transition-all mb-4"
            />
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              ...
              {["NRWP-1217", "LHR-1005", "KHI-2201"].map((id) => (
                <button key={id} onClick={() => { setFranchiseId(id); setError(""); }} className="text-[#0EA5E9]/50 text-xs font-mono hover:text-[#0EA5E9] transition-colors">
                  {id}
                </button>
              ))}
            </div>
          ...
          <>
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep("select")} className="flex-1 py-3 bg-white/5 border border-white/10 text-white/60 text-sm font-medium rounded-xl hover:bg-white/10 transition-all">
                Change
              </button>
              <button onClick={() => onVerified(verifiedId)} className="flex-1 py-3 bg-gradient-to-r from-[#0057FF] to-[#0EA5E9] text-white text-sm font-bold rounded-xl hover:shadow-[0_0_25px_rgba(0,87,255,0.3)] transition-all hover:scale-[1.02]">
                Continue to Login
              </button>
            </div>
          </>
