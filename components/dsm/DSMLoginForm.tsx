...
import { Eye, EyeOff, ArrowRight, KeyRound, User, Users } from "lucide-react";
import { useDSMData } from "@/lib/DSMDataContext";

interface DSMLoginFormProps {
  franchiseId: string;
}

export default function DSMLoginForm({ franchiseId }: DSMLoginFormProps) {
  const router = useRouter();
  const { dsmLogin } = useDSMData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const success = await dsmLogin(username, password);
      if (success) {
        router.push("/dsm/dashboard");
      } else {
      ...
      <div className="relative">
        ...
        <div className="bg-white/5 rounded-xl p-4 border border-white/5 mb-5">
          <div className="grid grid-cols-3 gap-3 text-center">
            <div>
              <p className="text-white/40 text-[10px] uppercase tracking-wider">Franchise</p>
