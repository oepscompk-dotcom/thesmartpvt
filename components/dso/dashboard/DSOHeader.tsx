...
export default function DSOHeader({ onMenuClick }: Props) {
  const { settings, auth, notifications } = useDSOData();
    ...
    <header className="h-16 bg-white/80 backdrop-blur-md border-b border-gray-200 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30">
      <div className="flex items-center gap-4">
        ...
        <div className="hidden sm:flex items-center gap-2 bg-gray-50 rounded-xl px-4 py-2.5 w-80 border border-gray-200 focus-within:border-[#0A2647]/30 focus-within:ring-2 focus-within:ring-[#0A2647]/10 transition-all">
          ...
          <input type="text" placeholder="Search activations, SIMs..." className="bg-transparent text-gray-900 text-sm placeholder:text-gray-400 focus:outline-none w-full" />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-3 pl-3 border-l border-gray-200">
          <div className="hidden sm:block text-right">
            <p className="text-gray-900 text-sm font-bold leading-tight">{settings.dsoName || auth.dsoName || "DSO"}</p>
            <p className="text-[#C8A951] text-[10px] font-semibold">{auth.dsoId}</p>
          </div>
          {logo ? (
            <div className="w-9 h-9 rounded-xl overflow-hidden ring-2 ring-[#C8A951]/20 shadow-sm">
            ...
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#0A2647] to-[#144272] flex items-center justify-center text-white font-bold text-xs shadow-md">
              {(settings.dsoName || auth.dsoName || "D").charAt(0).toUpperCase()}
            </div>
          )}
        </div>
      </div>
