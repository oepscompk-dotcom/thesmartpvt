...
const DOC_FIELDS: { key: string; label: string; path: (d: DSM) => string | undefined }[] = [
...
export default function DSMPage() {
  ...
  const [viewDSM, setViewDSM] = useState<DSM | null>(null);
  const [showDelete, setShowDelete] = useState<string | null>(null);

  const [salaryEdits, setSalaryEdits] = useState<Record<string, Partial<DSM>>>({});

  const applyFilters = (list: DSM[]) => {
    let result = list;
    if (statusFilter !== "All") {
  ...
  const allFiltered = applyFilters(dsms);
  const activeDSM = allFiltered.filter((d) => d.status === "Active");
  const inactiveDSM = allFiltered.filter((d) => d.status === "Inactive" || d.status === "Resigned");
  const suspendedDSM = allFiltered.filter((d) => d.status === "Suspended");
  ...
  const openEdit = (d: DSM) => {
    router.push("/franchise/dsm/create?edit=" + d.id);
  };
  ...
  const downloadSalarySlip = (emp: DSM, monthVal?: string) => {
    ...
    const v = (f: string) => (emp[f as keyof DSM] as number) || 0;
...
<\/script>
  ...
  const getSalaryValue = (d: DSM, field: string): number => {
    if (salaryEdits[d.id] && salaryEdits[d.id]![field as keyof DSM] !== undefined) {
      return salaryEdits[d.id]![field as keyof DSM] as number;
    }
    return (d[field as keyof DSM] as number) || 0;
  };

  const updateSalaryField = (dsmId: string, field: string, value: number) => {
    setSalaryEdits((prev) => ({
      ...prev,
      [dsmId]: { ...(prev[dsmId] || {}), [field]: value },
    }));
  };

  const saveSalaryRow = (d: DSM) => {
    const edits = salaryEdits[d.id];
    if (edits) {
      updateDSM(d.id, { ...d, ...edits });
      setSalaryEdits((prev) => {
        const next = { ...prev };
        delete next[d.id];
        return next;
  ...
  return (
    <div className="space-y-6">
      ...
      {viewDSM && <ViewModal dsm={viewDSM} onClose={() => setViewDSM(null)} />}
...
function DocumentsTab({
  ...
  data: DSM[];
  search: string;
  getDocCount: (d: DSM) => { uploaded: number; total: number };
  onView: (d: DSM) => void;
}) {
  return (
...
function SalaryTab({
  ...
  data: DSM[];
  search: string;
  salaryEdits: Record<string, Partial<DSM>>;
  getSalaryValue: (d: DSM, field: string) => number;
  updateSalaryField: (dsmId: string, field: string, value: number) => void;
  ...
  const getActivationsForDSM = (dsmId: string, month: string) => {
    const dsmActivations = activations.filter((a) => {
  ...
  const totalStats = useMemo(() => {
    ...
    data.forEach((d) => {
      const acts = getActivationsForDSM(d.id, calcMonth);
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 p-4">
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                {[
                                  { label: "New SIM-BVS (Rs.)", field: "newSimBvs" },
                                  { label: "New SIM-FCA (Rs.)", field: "newSimFca" },
                                  { label: "New SIM-IFCA (Rs.)", field: "newSimIfca" },
                                  { label: "MNP-BVS (Rs.)", field: "mnpBvs" },
                                  { label: "MNP-FCA (Rs.)", field: "mnpFca" },
                                  { label: "MNP-IFCA (Rs.)", field: "mnpIfca" },
                                  { label: "Repl-BVS (Rs.)", field: "replacementBvs" },
                                  { label: "Repl-FCA (Rs.)", field: "replacementFca" },
                                  { label: "Repl-IFCA (Rs.)", field: "replacementIfca" },
                                  { label: "BYN-BVS (Rs.)", field: "bynBvs" },
                                  { label: "BYN-FCA (Rs.)", field: "bynFca" },
                                  { label: "BYN-IFCA (Rs.)", field: "bynIfca" },
                                  { label: "Hike Commission (Rs.)", field: "hikeCommission" },
      ...
      >
        ...
        <div className="p-6 space-y-5">
          ...
          <Section title="Personal Information">
            <InfoRow label="Father Name" value={dsm.fatherName} />
            <InfoRow label="CNIC" value={dsm.cnic} />
            <InfoRow label="Mobile" value={dsm.mobile} />
            {dsm.email && <InfoRow label="Email" value={dsm.email} />}
            {dsm.dob && <InfoRow label="Date of Birth" value={dsm.dob} />}
            {dsm.gender && <InfoRow label="Gender" value={dsm.gender} />}
            {dsm.maritalStatus && <InfoRow label="Marital Status" value={dsm.maritalStatus} />}
            {dsm.nationality && <InfoRow label="Nationality" value={dsm.nationality} />}
            {dsm.address && <InfoRow label="Address" value={dsm.address} />}
          </Section>

          <Section title="Login Credentials">
            <InfoRow label="User ID" value={dsm.username} />
            <InfoRow label="Password" value={dsm.password} />
            <div className="mt-3">
              <button
                onClick={() => {
          ...
          <Section title="Employment">
            {dsm.designation && <InfoRow label="Designation" value={dsm.designation} />}
            {dsm.employmentType && <InfoRow label="Employment Type" value={dsm.employmentType} />}
            <InfoRow label="Joining Date" value={formatDateDDMMYYYY(dsm.joiningDate)} />
            <InfoRow label="Retailer ID" value={dsm.retailerId} />
            {dsm.reportingManager && <InfoRow label="Reporting Manager" value={dsm.reportingManager} />}
          </Section>

          <Section title="Salary & Compensation">
            <InfoRow label="Basic Salary" value={`PKR ${(dsm.salary || 0).toLocaleString()}`} />
            {dsm.fuelAllowance ? <InfoRow label="Fuel Allowance" value={`PKR ${(dsm.fuelAllowance || 0).toLocaleString()}`} /> : null}
            {dsm.mobileAllowance ? <InfoRow label="Mobile Allowance" value={`PKR ${(dsm.mobileAllowance || 0).toLocaleString()}`} /> : null}
            {dsm.dailyAllowance ? <InfoRow label="Daily Allowance" value={`PKR ${(dsm.dailyAllowance || 0).toLocaleString()}`} /> : null}
            {dsm.commission ? <InfoRow label="Commission" value={`${dsm.commission}%`} /> : null}
            {dsm.mnpCommission ? <InfoRow label="MNP Commission" value={`${dsm.mnpCommission}%`} /> : null}
            {dsm.replacementCommission ? <InfoRow label="Replacement Commission" value={`${dsm.replacementCommission}%`} /> : null}
            {dsm.bonus ? <InfoRow label="Bonus" value={`PKR ${(dsm.bonus || 0).toLocaleString()}`} /> : null}
          </Section>

          {dsm.bankName && (
            <Section title="Bank Details">
              <InfoRow label="Bank" value={dsm.bankName} />
              {dsm.accountTitle && <InfoRow label="Account Title" value={dsm.accountTitle} />}
              {dsm.accountNumber && <InfoRow label="Account Number" value={dsm.accountNumber} />}
              {dsm.iban && <InfoRow label="IBAN" value={dsm.iban} />}
            </Section>
          )}

          {(dsm.documents?.cnicFront || dsm.documents?.cnicBack || dsm.documents?.photo || dsm.agreements?.agreementPdf || dsm.guarantor?.name) && (
            <Section title="Documents & Guarantor">
              {dsm.documents?.cnicFront && <InfoRow label="CNIC Front" value="Uploaded" />}
              {dsm.documents?.cnicBack && <InfoRow label="CNIC Back" value="Uploaded" />}
              {dsm.documents?.photo && <InfoRow label="Photo" value="Uploaded" />}
              {dsm.agreements?.agreementNumber && <InfoRow label="Agreement #" value={dsm.agreements.agreementNumber} />}
              {dsm.agreements?.stampNumber && <InfoRow label="Stamp #" value={dsm.agreements.stampNumber} />}
              {dsm.guarantor?.name && <InfoRow label="Guarantor" value={dsm.guarantor.name} />}
            </Section>
          )}
        </div>
      </div>
