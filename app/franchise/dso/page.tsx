...
function SalaryTab({
  ...
  const getActivationsForDSO = (dsoId: string, month: string) => {
    ...
    return {
      ...
      bvs: dsoActivations.filter((a) => a.bvsStatus === "Completed").length,
      fca: dsoActivations.filter((a) => a.fcaStatus === "Completed").length,
      ifca: dsoActivations.filter((a) => a.ifcaStatus === "Completed").length,
      total: dsoActivations.length,
    };
  };

  const calcEarnings = (d: DSO, acts: { newSIM: number; mnp: number; replacement: number; byn: number; bvs: number; fca: number; ifca: number }) => {
