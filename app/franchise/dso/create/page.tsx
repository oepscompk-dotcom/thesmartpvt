...
export default function DSOCreatePage() {
  ...
  const existingDSO = isEditMode ? dso.find((d) => d.id === editId) : null;
  const [step, setStep] = useState(0);
  const [saved, setSaved] = useState(false);
  const [errors, setErrors] = useState<{ field: string; label: string; step: number }[]>([]);
  const [showErrors, setShowErrors] = useState(false);

  const getFreshDefaults = (): DSO => ({
    id: genId("DSO-" + auth.franchiseId.split("-")[0], dso.length + 1), name: "", fatherName: "", cnic: "", mobile: "", address: "",
    assignedDSM: "", joiningDate: new Date().toISOString().split("T")[0], salary: 0, commission: 0,
    username: genUsername("dso", dso.length + 1), password: genPassword(), status: "Active", photo: "",
    ...
    postalCode: "", employmentType: "Full Time", department: "Sales", designation: "Direct Sales Officer",
  ...
  const [form, setForm] = useState<DSO>(() => {
    const defaults = getFreshDefaults();
    if (existingDSO) {
      return {
        ...defaults,
        ...existingDSO,
        newSimLimits: { ...defaults.newSimLimits, ...existingDSO.newSimLimits },
        hlrSimLimits: { ...defaults.hlrSimLimits, ...existingDSO.hlrSimLimits },
        dailyTargets: { ...defaults.dailyTargets, ...existingDSO.dailyTargets },
        monthlyTargets: { ...defaults.monthlyTargets, ...existingDSO.monthlyTargets },
        attendanceSettings: { ...defaults.attendanceSettings, ...existingDSO.attendanceSettings },
        permissions: { ...defaults.permissions, ...existingDSO.permissions },
        documents: { ...defaults.documents, ...existingDSO.documents },
        agreements: { ...defaults.agreements, ...existingDSO.agreements },
        guarantor: { ...defaults.guarantor, ...existingDSO.guarantor },
      };
    }
    return defaults;
  });
        ...
        {step === 0 && (
          <div className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {renderField("DSO ID", "id", "text", { disabled: true })}
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {renderField("Designation", "designation", "select", { options: DESIG_DSO })}
              <DSMSearchSelect
                dsms={dsms}
                value={form.assignedDSM}
...
function DSMSearchSelect({ dsms, value, onChange }: { dsms: any[]; value: string; onChange: (v: string) => void }) {
