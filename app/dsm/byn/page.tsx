...
import {
  Hash, X, ArrowRight, ArrowLeft, CheckCircle, Trash2, Search,
} from "lucide-react";
...
export default function BYNPage() {
  ...
  const bynActivations = activations.filter((a) => a.type === "BYN");
  ...
  const handleSubmit = async () => {
    if (!selectedSimId || !form.desiredNumber) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <PageHeader
          breadcrumb={[{ label: "DSM" }, { label: "Activations" }, { label: "BYN" }]}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="text-left px-4 py-3 text-muted-foreground text-xs font-medium uppercase">Desired Number</th>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={12} className="px-6 py-12 text-center">
                    <Hash size={40} className="mx-auto mb-3 text-slate-300" />
                    <p className="text-sm text-muted-foreground">No BYN activations found</p>
                  </td>
                </tr>
              ) : (
                filtered.map((a, idx) => (
                  <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{a.simNumber}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-lg bg-white rounded-2xl border border-slate-200 mx-4 shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h3 className="text-lg font-bold text-foreground">Book Your Number</h3>
