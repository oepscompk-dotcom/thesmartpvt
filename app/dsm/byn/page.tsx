...
import { Hash, User, Phone, Smartphone, MessageSquare, CheckCircle2, Tag, ArrowLeft, ChevronRight, Signal } from "lucide-react";
...
export default function BYNRegistration() {
  ...
  useEffect(() => {
    ...
    const mySims = allSims.filter((s) => s.issuedToId === auth.dsmId && s.status === "Issued" && s.type === "new");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addActivation({
      id,
      type: "BYN",
      simNumber: form.msisdn || form.simNumber,
    ...
    try {
      if (auth.franchiseId) {
        ...
        if (stored) {
          const sims = JSON.parse(stored);
          setSimStockList(sims.filter((s: any) => s.issuedToId === auth.dsmId && s.status === "Issued" && s.type === "new"));
        }
      }
    } catch {}
    setShowSuccess(true);
  ...
  return (
    <div className="space-y-6">
      {showSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-8 shadow-2xl text-center max-w-sm mx-4 animate-bounce">
            ...
            <h3 className="text-gray-900 font-bold text-xl mb-2">BYN Submitted!</h3>
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">BYN Registration</h1>
          <p className="text-gray-500 text-sm">Bring Your Number registration</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">MSISDN</label>
              <input
                type="text"
                value={form.msisdn}
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          ...
          <textarea
            ...
            placeholder="Notes about this BYN registration..."
            rows={3}
