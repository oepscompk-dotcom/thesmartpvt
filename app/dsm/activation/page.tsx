...
import {
  Plus, X, Smartphone, ArrowRight, ArrowLeft, Trash2, Search, CheckCircle,
} from "lucide-react";
...
export default function NewSIMActivationPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const mySims = sims.filter((s: SIMStock) => s.issuedToId === auth.dsmId && s.status === "Issued" && s.type === "new");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <PageHeader
          breadcrumb={[{ label: "DSM" }, { label: "Activations" }, { label: "New SIM" }]}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="text-left px-4 py-3 text-muted-foreground text-xs font-medium uppercase">SIM Number</th>
                <th className="text-left px-4 py-3 text-muted-foreground text-xs font-medium uppercase hidden lg:table-cell">ICCID</th>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((a, idx) => (
                  <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <p className="text-sm font-mono font-medium text-foreground">{a.simNumber}</p>
                    </td>
                    <td className="px-4 py-3 hidden lg:table-cell text-muted-foreground text-xs font-mono">{(a as any).iccid || "\u2014"}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        <StatusPill label={derivedStatus(a)} tone={derivedStatus(a) === "Completed" ? "positive" : "warning"} />
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${typeColor(a.type)}`}>New</span>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-lg bg-white rounded-2xl border border-slate-200 mx-4 shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h3 className="text-lg font-bold text-foreground">New SIM Activation</h3>
            ...
            <div className="px-6 py-4 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
                  ...
                  <div className="grid grid-cols-3 gap-3 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-foreground">{form.simNumber}</p>
                    </div>
                    <div>
              ...
              <div>
                <label className="block mb-1 text-xs font-medium text-muted-foreground">Retailer ID</label>
                <Input
                  name="retailerId"
