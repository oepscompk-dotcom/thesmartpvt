...
export default function DSMPendingBVSPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        breadcrumb={[{ label: "DSM" }, { label: "Verify" }, { label: "BVS" }]}
            ...
            <React.Fragment key={step}>
              <div className="flex flex-col items-center gap-2">
                <div className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold ${
                  i === 0 ? "bg-brand-600 text-white" : "bg-slate-100 text-muted-foreground"
                }`}>
                  {i + 1}
                </div>
                <span className={`text-xs font-medium ${i === 0 ? "text-brand-700" : "text-muted-foreground"}`}>{step}</span>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <StatCard label="Total Completed" value={totalCompleted.length} icon={Fingerprint} iconClass="text-blue-600 bg-blue-50" />
      </div>
