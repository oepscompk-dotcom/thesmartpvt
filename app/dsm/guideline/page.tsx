...
const steps = {
  "new-sim": {
    ...
    steps: [
      ...
      { step: 2, title: "Select DSO", desc: "Choose the DSO who will handle this activation from the list of your assigned DSOs.", icon: Users, detail: "As DSM, you assign activations to your DSOs. The selected DSO will receive this activation in their queue." },
      { step: 3, title: "Enter Customer Details", desc: "Fill in customer name, mobile number, CNIC number, and address.", icon: User, detail: "All fields marked with * are mandatory. CNIC must be 13 digits." },
      { step: 4, title: "Select SIM Card", desc: "Choose an available SIM from the dropdown. SIMs with status In Stock are shown.", icon: CreditCard, detail: "Only SIMs available in stock will appear. Each SIM is uniquely identified by SIM Number." },
      { step: 5, title: "Select Device (Optional)", desc: "If a device is being issued with the SIM, select it from the device list.", icon: Smartphone, detail: "Device selection is optional. Only In Stock devices appear in the list." },
    ...
    flowDiagram: ["Dashboard", "Select DSO", "Customer Details", "SIM Selection", "Package Selection", "Submit", "Activation Complete"]
  },
  "mnp": {
    title: "MNP Process",
    icon: ArrowRightLeft,
    ...
    steps: [
      ...
      { step: 5, title: "Select New SIM", desc: "Choose an HLR SIM from stock for the ported number.", icon: CreditCard, detail: "MNP uses HLR (Home Location Register) SIMs, not new SIMs." },
      { step: 6, title: "Submit MNP Request", desc: "Submit the request. Status will show as Pending MNP approval.", icon: Clock, detail: "MNP typically takes 3-7 working days. Track status on the dashboard." },
    ],
    flowDiagram: ["Dashboard", "Select DSO", "MNP Form", "Porting Details", "Document Upload", "SIM Selection", "Submit Request", "MNP Processing"]
  },
  "replacement": {
    ...
    steps: [
      ...
      { step: 5, title: "Verify Identity", desc: "Verify customer identity using CNIC and biometric if available.", icon: Fingerprint, detail: "Biometric verification is recommended for security." },
      { step: 6, title: "Submit Replacement", desc: "Submit the replacement request. Old SIM deactivates immediately.", icon: CheckCircle2, detail: "The customer receives a new SIM with the same number." },
    ],
    flowDiagram: ["Dashboard", "Select DSO", "Replacement Form", "Customer Details", "SIM Selection", "Identity Verification", "Submit", "Replacement Complete"]
  },
  "byn": {
    ...
    steps: [
      ...
      { step: 5, title: "Enter Network Info", desc: "Select network operator and enter any required authorization codes.", icon: Wifi, detail: "BYN links the device IMEI to the SIM for network registration." },
      { step: 6, title: "Submit BYN Request", desc: "Submit the BYN registration. Device will be registered on the network.", icon: CheckCircle2, detail: "BYN registration is usually instant. Device can make calls immediately." },
    ],
    flowDiagram: ["Dashboard", "Select DSO", "BYN Form", "Device Details", "SIM Selection", "Network Info", "Submit", "Registration Complete"]
  }
};
...
const verificationSteps = [
  {
    ...
    color: "amber",
    steps: [
      "Open Pending BVS from DSM menu",
      "View list of activations pending BVS across all DSOs",
  ...
  },
  {
    ...
    color: "blue",
    steps: [
      "Open Pending FCA from DSM menu",
  ...
  },
  {
    ...
    color: "purple",
    steps: [
      "Open Pending IFCA from DSM menu",
...
const dsoManagementSteps = [
  { step: 1, title: "View DSO List", desc: "Access the complete list of DSOs under your supervision with their performance metrics.", icon: Users, detail: "View all assigned DSOs with their current workload, pending activations, completion rates, and status (Active/Inactive)." },
  { step: 2, title: "Add New DSO", desc: "Register a new DSO and assign them to your distribution network.", icon: Plus, detail: "Enter DSO details including name, CNIC, contact number, and area assignment. The DSO receives login credentials automatically." },
  { step: 3, title: "Monitor Performance", desc: "Track individual and team performance through analytics and reports.", icon: BarChart3, detail: "Monitor activation counts, verification completion rates, average processing time, and target achievement for each DSO." },
...
export default function DSMGuidelinePage() {
  const [activeSection, setActiveSection] = useState<Section>("overview");
  const [expandedStep, setExpandedStep] = useState<number | null>(null);
  const [expandedVerification, setExpandedVerification] = useState<number | null>(null);
  const [expandedDsoMgmt, setExpandedDsoMgmt] = useState<number | null>(null);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              ...
              <p className="text-gray-600 text-sm leading-relaxed mb-4">
                As a DSM, managing your DSO team is critical to achieving activation targets. Use this section to view your DSO roster, add new team members, monitor performance metrics, and manage territory assignments.
              </p>
              <div className="flex flex-wrap items-center gap-2">
                {["View DSOs", "Add New", "Monitor", "Assign Tasks"].map((step, i, arr) => (
                  <div key={step} className="flex items-center gap-2">
                    <div className="flex flex-col items-center gap-1">
                      <div className="w-10 h-10 rounded-xl bg-green-500 text-white flex items-center justify-center text-xs font-bold">
                        {i + 1}
                      </div>
                      <span className="text-[10px] font-medium text-gray-600 text-center w-16">{step}</span>
                    </div>
