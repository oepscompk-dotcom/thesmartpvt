...
export default function CompanyDashboardPage() {
  ...
  const statCards = [
    { label: "Total Franchises", value: totalFranchises, sub: `${active} active · ${pending} pending`, icon: Building2, light: "bg-blue-50", color: "text-blue-600", trend: `+${active}`, up: true },
    { label: "Total Staff", value: totalStaff, sub: `${staffPerFranchise.toFixed(1)} avg per franchise`, icon: Users, light: "bg-purple-50", color: "text-purple-600", trend: `${attendanceRate}%`, up: attendanceRate >= 70 },
    { label: "SIM Inventory", value: totalSIMs, sub: `${totalNewSims} New · ${totalHlrSims} HLR`, icon: Smartphone, light: "bg-cyan-50", color: "text-cyan-600", trend: `${totalDevices} devices`, up: true },
    { label: "Total Revenue", value: `PKR ${(totalRevenue / 100000).toFixed(1)}L`, sub: `${todayActivations} activations today`, icon: DollarSign, light: "bg-amber-50", color: "text-amber-600", trend: `${totalFranchises} franchises`, up: true },
  ];
  ...
  const quickActions = [
    ...
    { label: "Settings", href: "/company/settings", icon: Settings, color: "bg-slate-50 text-slate-600" },
  ...
  return (
    <div className="space-y-6">
      {/* Gradient Header — retained navy brand statement */}
      <div className="bg-gradient-to-r from-[#0A2647] via-[#144272] to-[#205295] rounded-xl p-6 text-white relative overflow-hidden">
      ...
      {/* Tab Navigation — underline style per guide */}
      <div className="flex items-center gap-6 border-b border-slate-200">
        {[
          ...
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            className={`-mb-px inline-flex items-center gap-1.5 border-b-2 pb-3 text-sm transition-all ${activeTab === tab.key ? "border-brand-600 font-semibold text-brand-700" : "border-transparent font-medium text-muted-foreground hover:border-slate-300 hover:text-foreground"}`}>
      ...
      {activeTab === "overview" && (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {statCards.map((s) => (
              <div key={s.label} className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm hover:shadow-md transition-all group">
                <div className="flex items-start justify-between mb-3">
                  <div className={`h-10 w-10 rounded-xl ${s.light} flex items-center justify-center group-hover:scale-110 transition-transform`}>
                ...
                <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mt-0.5">{s.label}</p>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {secondaryStats.map((s) => (
              <div key={s.label} className="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm hover:shadow-md transition-all">
                <div className="flex items-center gap-3">
                  <div className={`h-10 w-10 rounded-xl ${s.light} flex items-center justify-center`}><s.icon size={18} className={s.color} /></div>
                  <div>
                    <p className="text-2xl font-semibold tracking-tight text-slate-900">{s.value}</p>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">{s.label} {s.sub && `· ${s.sub}`}</p>
                    {s.pct > 0 && (
          ...
          <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm">
            <h3 className="text-slate-900 font-bold text-sm mb-4 flex items-center gap-2"><Zap size={16} className="text-brand-600" /> Quick Actions</h3>
            <div className="grid grid-cols-4 lg:grid-cols-8 gap-3">
              {quickActions.map((a) => (
                <button key={a.label} onClick={() => a.onClick ? a.onClick() : router.push(a.href || "#")}
                  className="rounded-xl border border-slate-200/80 bg-white p-4 flex flex-col items-center gap-2 hover:shadow-md transition-all">
                  <div className={`h-10 w-10 rounded-xl ${a.color.split(" ")[0]} flex items-center justify-center`}>
                    <a.icon size={18} className={a.color.split(" ")[1]} />
                  </div>
                  <span className="text-[11px] font-semibold text-slate-700 whitespace-nowrap">{a.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="rounded-xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
                <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4 border-b border-slate-100 flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-slate-900 flex items-center gap-2"><Award size={16} className="text-brand-600" /> Top Franchises <span className="text-slate-400 font-normal text-xs">({totalFranchises} total)</span></h3>
                  <button onClick={() => router.push("/company/franchises")} className="text-xs font-medium text-brand-700 hover:underline flex items-center gap-1">View All <ChevronRight size={12} /></button>
                </div>
                <div className="p-4">
                  {topFranchises.length > 0 ? (
                    <div className="space-y-3">
                      {topFranchises.map((f, i) => (
                        <div key={f.id} className={`flex items-center gap-4 p-3 rounded-xl transition-all ${i === 0 ? "bg-amber-50 border border-amber-200" : "hover:bg-slate-50"}`}>
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black ${i === 0 ? "bg-amber-400 text-white" : i === 1 ? "bg-slate-300 text-white" : i === 2 ? "bg-orange-400 text-white" : "bg-slate-100 text-slate-500"}`}>{i + 1}</div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <p className="text-slate-900 text-sm font-bold truncate">{f.name}</p>
                              {i === 0 && <Star size={12} className="text-amber-400 fill-amber-400" />}
                            </div>
                            <p className="text-slate-400 text-xs">{f.id} &middot; {f.city} &middot; {f.staff} staff</p>
                          </div>
                          <div className="text-right">
                            <p className="text-slate-900 text-sm font-bold">PKR {(f.revenue / 1000).toFixed(1)}K</p>
                            <p className="text-slate-400 text-[10px]">{f.todayActivations} today</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-slate-400 text-sm text-center py-8">No franchises found</p>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
                <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2"><PieChart size={16} className="text-brand-600" /> Franchise Status Distribution</h3>
                  ...
                  {[
                    ...
                    <div key={s.label}>
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-slate-700 text-sm flex items-center gap-1.5">{s.icon} {s.label}</span>
                        <div className="flex items-center gap-2"><span className={`text-sm font-bold ${s.label === "Active" ? "text-green-600" : s.label === "Pending" ? "text-yellow-600" : "text-red-600"}`}>{s.count}</span><span className="text-slate-400 text-xs">({s.pct}%)</span></div>
                      </div>
                      <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full ${s.color} rounded-full transition-all duration-1000`} style={{ width: `${s.pct}%` }} /></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
                <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2"><MapPin size={16} className="text-brand-600" /> By Province</h3>
                <div className="space-y-3">
                  {Object.entries(franchises.reduce<Record<string, number>>((acc, f) => { acc[f.province] = (acc[f.province] || 0) + 1; return acc; }, {})).sort((a, b) => b[1] - a[1]).map(([prov, count]) => {
                    ...
                    return (
                      <div key={prov} className="flex items-center gap-3">
                        <span className="text-slate-600 text-sm w-20 truncate">{prov}</span>
                        <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-rose-500 rounded-full" style={{ width: `${pct}%` }} /></div>
                        <span className="text-slate-900 text-sm font-bold w-8 text-right">{count}</span>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
                <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4 border-b border-slate-100 flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-slate-900 flex items-center gap-2"><DollarSign size={16} className="text-brand-600" /> Financial Summary</h3>
                  <button onClick={() => router.push("/company/reports")} className="text-xs font-medium text-brand-700 hover:underline flex items-center gap-1">Details <ChevronRight size={12} /></button>
                </div>
                <div className="p-4 space-y-3">
                  ...
                  <div className="pt-2 border-t border-slate-100">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-800 text-sm font-bold">Net Profit</span>
              ...
              <div className="rounded-xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
                <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4 border-b border-slate-100"><h3 className="text-sm font-semibold text-slate-900 flex items-center gap-2"><Wifi size={16} className="text-brand-600" /> SIM Network Distribution</h3></div>
                <div className="p-6">
                    ...
                    <div className="space-y-4">
                      {networkDist.map(([network, count]) => {
                        ...
                        return (
                          <div key={network}>
                            <div className="flex items-center justify-between mb-1.5">
                              <div className="flex items-center gap-2"><Wifi size={14} className={network === "Jazz" ? "text-red-500" : network === "Telenor" ? "text-blue-500" : network === "Ufone" ? "text-green-500" : "text-purple-500"} /><span className="text-slate-700 text-sm font-medium">{network}</span></div>
                              <span className="text-slate-500 text-xs">{count} SIMs ({pct}%)</span>
                            </div>
                            <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-700 ${netColors[network] || "bg-slate-400"}`} style={{ width: `${pct}%` }} /></div>
                          </div>
                        );
                      })}
                    </div>
                  ) : (
                    <p className="text-slate-400 text-sm text-center py-4">No SIM data</p>
                  )}
                </div>
              </div>
              ...
              {/* Stats Overview */}
              <div className="rounded-xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
                <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4 border-b border-slate-100"><h3 className="text-sm font-semibold text-slate-900 flex items-center gap-2"><BarChart3 size={16} className="text-brand-600" /> Quick Facts</h3></div>
                <div className="p-4 space-y-3">
                  {[
                    ...
                    <div key={s.label} className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 bg-slate-50/50">
                      <div className="w-9 h-9 rounded-lg bg-white/80 flex items-center justify-center"><s.icon size={16} className={s.color} /></div>
                      <div className="flex-1"><p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">{s.label}</p><p className="text-slate-900 text-sm font-bold">{s.value}</p></div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.label} className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  <div className={`h-10 w-10 rounded-xl ${s.light} flex items-center justify-center`}><s.icon size={20} className={s.color} /></div>
                </div>
                <p className="text-2xl font-semibold tracking-tight text-slate-900">{s.value}</p>
                <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mt-0.5">{s.label}</p>
                <p className="text-slate-400 text-[10px] mt-1">{s.sub}</p>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="rounded-xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
              <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4 border-b border-slate-100"><h3 className="text-sm font-semibold text-slate-900 flex items-center gap-2"><Activity size={16} className="text-brand-600" /> Activations by Franchise</h3></div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-slate-100 bg-slate-50">
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Franchise</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">Total</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">Today</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">Pending</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">Rate</th>
                  </tr></thead>
                  <tbody>
                    {franchises.map((f) => {
                      ...
                      return (
                        <tr key={f.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3"><p className="text-slate-900 text-sm font-medium">{f.name}</p><p className="text-slate-400 text-[10px]">{f.id}</p></td>
                          <td className="px-4 py-3 text-center text-slate-700 font-medium">{d?.totalActivations || 0}</td>
                          <td className="px-4 py-3 text-center text-slate-700 font-medium">{f.todayActivations}</td>
                          <td className="px-4 py-3 text-center text-slate-700 font-medium">{d?.pendingActivations || 0}</td>
            ...
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2"><Layers size={16} className="text-brand-600" /> SIM Inventory by Status</h3>
              {(() => {
                const byStatus: Record<string, number> = {};
                Object.values(detailMap).forEach((d) => Object.entries(d.simsByStatus).forEach(([st, c]) => { byStatus[st] = (byStatus[st] || 0) + c; }));
                const cmap: Record<string, string> = { Issued: "bg-amber-500", Active: "bg-green-500", Verified: "bg-blue-500", Returned: "bg-red-500", "In Stock": "bg-slate-400" };
                  ...
                  <div className="space-y-4">
                    {Object.entries(byStatus).sort((a, b) => b[1] - a[1]).map(([st, count]) => {
                      ...
                      return (
                        <div key={st}>
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-slate-700 text-sm font-medium">{st}</span>
                            <span className="text-slate-500 text-xs">{count} ({pct}%)</span>
                          </div>
                          <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${cmap[st] || "bg-slate-400"}`} style={{ width: `${pct}%` }} /></div>
                        </div>
                      );
                    })}
                  </div>
          ...
          <div className="rounded-xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
            <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4 border-b border-slate-100"><h3 className="text-sm font-semibold text-slate-900 flex items-center gap-2"><MapPin size={16} className="text-brand-600" /> Franchise Performance by Province</h3></div>
            <div className="p-6">
              {(() => {
                ...
                return Object.entries(byProv).sort((a, b) => b[1].revenue - a[1].revenue).map(([prov, data]) => {
                  ...
                  return (
                    <div key={prov} className="flex items-center gap-4 py-3 border-b border-slate-100 last:border-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-slate-900 text-sm font-medium">{prov}</p>
                        <p className="text-slate-400 text-xs">{data.count} franchises &middot; {data.staff} staff</p>
                      </div>
                      <div className="text-right">
                        <p className="text-slate-900 text-sm font-bold">PKR {(data.revenue / 1000).toFixed(1)}K</p>
                        <div className="w-16 h-1.5 bg-slate-100 rounded-full mt-1 ml-auto overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: `${revPct}%` }} /></div>
                      </div>
                    </div>
                  );
                });
      ...
      {activeTab === "finance" && (
        <div className="space-y-6">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2"><Wallet size={16} className="text-brand-600" /> Wallet by Franchise</h3>
                ...
                {[...franchises].sort((a, b) => (detailMap[b.id]?.walletBalance || 0) - (detailMap[a.id]?.walletBalance || 0)).map((f) => {
                  ...
                  return (
                    <div key={f.id} className="flex items-center gap-2">
                      <span className="text-slate-600 text-xs w-16 truncate">{f.name}</span>
                      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: `${pct}%` }} /></div>
                      <span className="text-slate-900 text-xs font-bold w-20 text-right">PKR {bal.toLocaleString()}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2"><Banknote size={16} className="text-brand-600" /> Revenue by Franchise</h3>
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {[...franchises].sort((a, b) => b.revenue - a.revenue).map((f) => {
                  ...
                  return (
                    <div key={f.id} className="flex items-center gap-2">
                      <span className="text-slate-600 text-xs w-16 truncate">{f.name}</span>
                      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-amber-500 rounded-full" style={{ width: `${pct}%` }} /></div>
                      <span className="text-slate-900 text-xs font-bold w-20 text-right">PKR {(f.revenue / 1000).toFixed(1)}K</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2"><CreditCard size={16} className="text-brand-600" /> Payroll by Franchise</h3>
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {[...franchises].sort((a, b) => (detailMap[b.id]?.totalPayroll || 0) - (detailMap[a.id]?.totalPayroll || 0)).map((f) => {
                  ...
                  return (
                    <div key={f.id} className="flex items-center gap-2">
                      <span className="text-slate-600 text-xs w-16 truncate">{f.name}</span>
                      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-purple-500 rounded-full" style={{ width: `${pct}%` }} /></div>
                      <span className="text-slate-900 text-xs font-bold w-20 text-right">PKR {payroll.toLocaleString()}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2"><Award size={16} className="text-emerald-500" /> Most Profitable Franchises</h3>
              <div className="space-y-3">
                {[...franchises].map((f) => {
                  const d = detailMap[f.id];
                  if (!d) return null;
                  ...
                  return (
                    <div key={f.id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 transition-all">
                      <div className="flex-1 min-w-0"><p className="text-slate-900 text-sm font-medium truncate">{f.name}</p></div>
            ...
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2"><Activity size={16} className="text-brand-600" /> Financial Breakdown by Franchise</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-slate-100 bg-slate-50">
                    <th className="px-2 py-2 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Franchise</th>
                    <th className="px-2 py-2 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Income</th>
                    <th className="px-2 py-2 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Expenses</th>
                    <th className="px-2 py-2 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Net</th>
                  </tr></thead>
                  <tbody>
                    {franchises.map((f) => {
                      const d = detailMap[f.id];
                      ...
                      return (
                        <tr key={f.id} className="border-b border-slate-100 hover:bg-slate-50">
                          <td className="px-2 py-2 text-slate-700 text-xs font-medium">{f.name}</td>
                          <td className="px-2 py-2 text-right text-slate-700 text-xs">PKR {d.totalIncome.toLocaleString()}</td>
                          <td className="px-2 py-2 text-right text-slate-700 text-xs">PKR {d.totalExpenses.toLocaleString()}</td>
