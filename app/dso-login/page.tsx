...
export default function DSOLoginPage() {
  ...
  const loginFlow = ["Franchise ID", "Employee Login", "Authentication", "DSO Dashboard"];
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <header className="px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              ...
              <span className="text-[#00C2FF]/60 text-[10px] font-medium">A Step Towards a New Horizon</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-white/50 text-xs font-medium">DSO</span>
          </div>
        </header>
          ...
          <div className="w-full max-w-7xl grid lg:grid-cols-[1fr_420px] gap-10 lg:gap-16 items-center">
            ...
            <div className={`hidden lg:block transition-all duration-1000 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
              <div className="mb-8">
                ...
                <h2 className="text-xl text-white/80 font-semibold mb-3">DSO Portal</h2>
                <p className="text-white/50 text-sm leading-relaxed max-w-md">
                  Access your field operations, track activations, manage GPS attendance, and view daily targets from one secure portal.
                </p>
              </div>
                  ...
                  {loginFlow.map((step, i) => (
                    <div key={step} className="flex items-center gap-2">
                      <div className="glass-card rounded-lg px-3 py-2 flex items-center gap-2">
                        <span className="w-5 h-5 rounded-full bg-[#00C2FF]/20 text-[#00C2FF] text-xs font-bold flex items-center justify-center">{i + 1}</span>
            ...
            <div className={`w-full max-w-[420px] mx-auto space-y-4 transition-all duration-1000 delay-300 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
              ...
              <div className="lg:hidden text-center mb-4">
                ...
                <p className="text-[#00C2FF] text-xs font-medium">DSO PORTAL</p>
                <p className="text-white/30 text-[10px] mt-1">A Step Towards a New Horizon</p>
              </div>
        ...
        <footer className="px-6 py-4 text-center">
          <p className="text-white/20 text-xs">{companyName || "THE SMART ERP"} &bull; Direct Sales Officer Portal</p>
          <p className="text-white/15 text-[10px] mt-1">&copy; 2026 {companyName || "THE SMART Pvt. Ltd."}</p>
        </footer>
      </div>
    </div>
  );
