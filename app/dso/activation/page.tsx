...
import {
  Plus, X, Smartphone, CheckCircle, Clock, ArrowRight, Trash2,
} from "lucide-react";
...
export default function NewSIMActivationPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const mySims = sims.filter((s: SIMStock) => s.issuedToId === auth.dsoId && s.status === "Issued" && s.type === "new");
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
        <PageHeader
          breadcrumb={[{ label: "DSO Dashboard", href: "/dso" }, { label: "New SIM Activation" }]}
          title="New SIM Activation"
          description="Create and manage new SIM activations"
          actions={
            <Button onClick={() => setShowModal(true)}>
              <Plus size={16} /> <span className="hidden sm:inline">New Activation</span>
            </Button>
          }
        />

        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <StatCard label="Total Activations" value={newSIMActivations.length} icon={Smartphone} />
          <StatCard label="Pending Verification" value={newSIMActivations.filter((a) => a.status !== "Completed").length} icon={Clock} iconClass="text-amber-600 bg-amber-50" />
          <StatCard label="Completed" value={newSIMActivations.filter((a) => a.status === "Completed").length} icon={CheckCircle} iconClass="text-green-600 bg-green-50" />
        </div>

        <Card>
          <div className="px-4 py-4 sm:px-6">
            <h2 className="mb-4 text-sm font-semibold text-foreground">Activation Workflow</h2>
        ...
        <Card>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">SIM Number</th>
                  <th className="text-left px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground hidden lg:table-cell">ICCID</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {pagedFiltered.map((a, idx) => (
                  <tr key={a.id} className="transition-colors hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 font-mono text-sm font-medium text-foreground">{a.simNumber}</td>
                    <td className="hidden px-4 py-3 font-mono text-xs text-muted-foreground lg:table-cell">{a.iccid}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        <StatusPill label={derivedStatus(a)} tone={derivedStatus(a) === "Completed" ? "positive" : "warning"} />
                        <span className={`rounded px-2 py-0.5 text-[10px] font-bold ${typeColor(a.type)}`}>{simTypeLabel(a.type)}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => { if (confirm("Delete this activation?")) deleteActivation(a.id); }} className="text-red-600 hover:bg-red-50 hover:text-red-700"><Trash2 size={14} /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          {pagedFiltered.length === 0 && (
            <EmptyState icon={Smartphone} title="No activations found" description="Create a new activation or adjust your filters." />
          )}
          {filtered.length > 0 && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
            <div className="bg-white mx-4 w-full max-w-lg rounded-lg border border-slate-200 shadow-2xl">
              <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <h3 className="text-base font-semibold text-foreground">New SIM Activation</h3>
              ...
              <div className="max-h-[60vh] overflow-y-auto space-y-4 px-6 py-4">
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-muted-foreground">Retailer ID</label>
                  <Input
                    name="retailerId"
                    value={form.retailerId}
                    readOnly
                    className="cursor-not-allowed bg-slate-100 text-muted-foreground"
                  />
                </div>
                <div>
