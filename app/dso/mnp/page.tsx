...
import {
  ArrowLeftRight, X, CheckCircle, Clock, ArrowRight, Trash2,
} from "lucide-react";
...
export default function MNPPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
        <PageHeader
          breadcrumb={[{ label: "DSO Dashboard", href: "/dso" }, { label: "MNP" }]}
          title="MNP (Mobile Number Portability)"
          description="Port numbers across networks"
          actions={
            <Button onClick={() => setShowModal(true)}>
              <ArrowLeftRight size={16} /> <span className="hidden sm:inline">New MNP</span>
            </Button>
          }
        />

        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <StatCard label="Total MNP" value={mnpActivations.length} icon={ArrowLeftRight} />
          <StatCard label="Pending Verification" value={mnpActivations.filter((a) => a.status !== "Completed").length} icon={Clock} iconClass="text-amber-600 bg-amber-50" />
          <StatCard label="Completed" value={mnpActivations.filter((a) => a.status === "Completed").length} icon={CheckCircle} iconClass="text-green-600 bg-green-50" />
        </div>

        <Card>
          <div className="px-4 py-4 sm:px-6">
            <h2 className="mb-4 text-sm font-semibold text-foreground">MNP Workflow</h2>
        ...
        <Card>
                  ...
                  <tr key={a.id} className="transition-colors hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => { if (confirm("Delete this MNP activation?")) deleteActivation(a.id); }} className="text-red-600 hover:bg-red-50 hover:text-red-700"><Trash2 size={14} /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          {pagedFiltered.length === 0 && (
            <EmptyState icon={ArrowLeftRight} title="No MNP activations found" description="Create a new MNP activation or adjust your filters." />
          )}
          {filtered.length > 0 && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
            <div className="bg-white mx-4 w-full max-w-lg rounded-lg border border-slate-200 shadow-2xl">
              <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <h3 className="text-base font-semibold text-foreground">New MNP Activation</h3>
              ...
              <div className="max-h-[60vh] overflow-y-auto space-y-4 px-6 py-4">
                  ...
                  <div className="rounded-lg border border-brand-100 bg-brand-50 p-4">
                    ...
                    <div className="grid grid-cols-3 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-brand-700">{form.newNetwork}</p>
                      </div>
                      <div>
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-muted-foreground">Current Network</label>
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-muted-foreground">New Network (must differ)</label>
                  <Select name="newNetwork" value={form.newNetwork} disabled className="cursor-not-allowed bg-slate-100 text-muted-foreground">
